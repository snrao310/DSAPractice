 ...
 *
 */
public class BurstBaloonLeetCode {

    //Hardest Problem seen to date on LeetCode. Very very non intuitive. Check the explanation
    //below by user @dietpepsi to understand
    public static int maxCoins(int[] nums) {
        ...
        arr[0]=arr[n++]=1;
        return burst(arr,new int[n][n],0,n-1);
    }
    ...
    private static int burst(int[] arr, int[][] dp, int left, int right) {
        ...
        for(int i=left+1;i<right;i++){
            max=Math.max(max, arr[left]*arr[i]*arr[right] + burst(arr,dp,left,i) + burst(arr,dp,i,right));
        }
        dp[left][right]=max;
        return max;
    }
...
Be Naive First
 ...
 Well, we can find that for any balloons left the maxCoins does not depends on the balloons already bursted. This
 indicate that we can use memorization (top down) or dynamic programming (bottom up) for all the cases from small
 numbers of balloon until n balloons. How many cases are there? For k balloons there are C(n, k) cases and for each
 case it need to scan the k balloons to compare. The sum is quite big still. It is better than O(n!) but worse than
 O(2^n).
 ...
 Well, the nature way to divide the problem is burst one balloon and separate the balloons into 2 sub sections one on
 the left and one one the right. However, in this problem the left and right become adjacent and have effects on the
 maxCoins in the future.
 ...
 of all to burst, the left and right section now has well defined boundary and do not affect each other! Therefore we
 can do either recursive method with memoization or dp.

 Final

 Here comes the final solutions. Note that we put 2 balloons with 1 as boundaries and also burst all the zero balloons
 in the first round since they won't give any coins.
