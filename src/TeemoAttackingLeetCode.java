 ...
 *
 */
public class TeemoAttackingLeetCode {

    public static int findPoisonedDuration(int[] timeSeries, int duration) {
        if(timeSeries.length==0) return 0;
        int prevEnd=timeSeries[0]+duration, time=duration;
        for(int i=1;i<timeSeries.length;i++){
            int a=timeSeries[i];
            time+=(a>prevEnd)?duration:duration-(prevEnd-a);
            prevEnd=a+duration;
        }
        return time;
    }
    ...
    public static void main(String[] args) {
        System.out.println(findPoisonedDuration(new int[]{1,2,3,6,9,13,14,15}, 2));
